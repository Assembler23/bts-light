 ...
 *    `teamOnSide` (wie monitor.html). **`teamOnSide` ist in beiden Zweigen
 *    Pflicht** — ohne es nie ein Punkt, auch wenn `serving.team` gesetzt
 *    ist: anders als `servingInfo` in monitor.html ordnet die Tafel den
 *    Punkt einer SEITE zu und müsste ohne Seiteninfo raten. Nach der
...
export function tafelSeiten(sets, courtState, spiegel) {
  ...
  // Geistersatz nur bei regulärem Spielende streichen — im laufenden Spiel
  // ist 0:0 der echte Beginn eines Satzes. Bei Aufgabe (`retired`) ist der
  // letzte Eintrag — auch ein 0:0 in der Satzpause — der unvollständige
  // Satz, der groß steht und nicht zählt; der bleibt also stehen.

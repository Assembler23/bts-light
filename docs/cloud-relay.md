...
**Liveness liegt beim Slave.** Hinter einem Träger misst der Relay nur noch
den Träger (Ping/Stale wie beim Host). Stirbt ein einzelnes Gerät, muss der
Slave `streamClose` senden — er hält die echte Verbindung dorthin und sieht
den Abriss sofort, schneller als die 15 s `TABLET_STALE`. Bricht der Träger

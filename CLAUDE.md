...
| **Schiedsrichtermanagement** (`btp/model.rs` `BtpOfficial`/`official_list`/`official1_id`/`official2_id`, `config.rs` `OfficialsConfig`, `tablet/officials.rs` `OfficialsStore` (turniergebundene `officials-state.json`), `state.rs` `officials_store()`, Schiedsrichter-Abschnitt im `SetupWizard`; im Aufbau — weitere Bausteine folgen je Spec-Schritt) | `docs/schiedsrichter-management.md` · Spec `docs/features/schiedsrichter-management.md` |

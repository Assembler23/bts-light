...
- `android/` — Kotlin-Kiosk-App für die Zähl-Tablets (eigenes
  Gradle-Projekt, nicht Teil des Cargo-Workspace).
...
| **Zeilenfarbe aus BTP („Hervorheben")** (`btp/model.rs` `BtpMatch.highlight`/`highlight_from`, `btp/proto.rs` `HighlightEntry.highlight`/`HIGHLIGHT_PREP_CALL`, `sync.rs` `highlight_desired`/`highlight_entries` (Handfarbe hat Vorrang), `tablet/state.rs` `remember_highlight`/`apply_highlight_echo`/`HIGHLIGHT_ECHO_MS`, `tablet/tl.rs` `execute_highlight_action` + `highlight` an `TlMatch`/`TlOpenMatch`/`TlCourt` + `TlState.can_set_highlight`, `relay-proto` `TlAction::SetHighlight`/`MAX_HIGHLIGHT`, `HL_FARBEN`/`.hl-N`/`hl-pick` in `assets/tl.html`, Messwerkzeug `tests/btp_highlight_probe.rs`) | `docs/features/tl-zeilenfarbe.md` (Spec) · `docs/turnierleitung-web.md` (Bedienung) · `docs/btp_protocol.md` (Werte, Messung) · `docs/preparation.md` (P1) · `docs/cloud-relay.md` (Wire-Ebene) · ADR 0056 |
...
| **Aufruf-Uhr am Tablet + Kopfzeile in der Seitenwahl** (`src/io/aufrufUhr.mjs` + `scripts/test-aufruf-uhr.mjs` + Inline-Kopie `aufrufUhr`/`feldAusHealth`/`spielLaeuft`/`renderAufrufUhr` in `assets/tablet.html`, `syncServerClock` mit `court=`-Filter + `healthNachfragen`, `#setup-modal`/`--kopf-hoehe`) | `docs/tablet.md` (Match-Setup + „Aufruf-Uhr in der Kopfzeile") · `docs/court-monitor.md` (TV-Seite der Uhr) |
...
| **Anordnung vorn/hinten am Zähl-Tablet** (`assets/tablet.html` `.play.uebereinander` + `.court-lines-hoch`, `anordnungWahl`/`uebereinander()`/`seiteWort`/`anordnungAnwenden`/`naechsteAnordnung`, Inline-Kopie von `anordnungAusQuery`/`effektiveAnordnung` aus `src/io/tafelSeiten.mjs`) | `docs/tablet.md` („Zählen hinter dem Feld") · Spec `docs/features/zaehltafel-anzeige-huelle.md` (Erweiterung 06.09.2026) |
...
| **Zähltafel + Anzeige-Hülle** (`assets/tafel.html` (Anordnung `nebeneinander`/`uebereinander` + `auto` nach Geräte-Ausrichtung, `?anordnung=`), `src/io/tafelSeiten.mjs` (`tafelSeiten`, `anordnungAusQuery`, `effektiveAnordnung`) + `scripts/test-tafel-seiten.mjs`, `tafel_page`/`render_tafel_html` + `CourtTafel`-Arm in `tablet/server.rs` **und** `relay/`, `relay-proto` `MonitorTarget::CourtTafel`, Option `tafel:<id>` in `pages/CourtMonitorPanel.tsx`; PR 2: `assets/anzeige.html`, `src/io/anzeigeZiel.mjs` + `scripts/test-anzeige-ziel.mjs`, `anzeige_page`/`render_anzeige_html`/`bedien_pin` in `tablet/server.rs`, `/{ns}/anzeige` + `occupied` in `courts_list` in `relay/`, Einstiege in `assets/tablet.html`; **Ansicht per Tipp** (13.09.2026): `ANSICHTEN`/`ansichtParameter`/`ansichtAusParametern`/`naechsteAnsicht` in `anzeigeZiel.mjs`, `postMessage` `bts-anzeige:naechste-ansicht` aus `tafel.html`, `ansichtWeiter`/`#ansicht-etikett` in `anzeige.html`) | `docs/features/zaehltafel-anzeige-huelle.md` (Spec) · `docs/court-monitor.md` (Layout, Zuweisung) · `docs/tablet.md` (Hülle, PR 2) · `docs/cloud-relay.md` (Route, Deploy-Reihenfolge) · ADR 0055 |
...
| **Mehrere Liveticker je Verband** (`config.rs` `AppConfig.tournament_uuid` + `kanonische_guid`/`spiegele_turnier_guid`, `badhub/payload.rs` `tournament_uuid` in `TsetEvent`/`SchedEvent`/`TupdateMessage`/`CheckinBrandingMessage` + `build_tupdate`-Parameter, `badhub/diff.rs`, `tablet/server.rs` `handle_score`, `commands.rs` `pruefe_startbedingungen`/`keep_host_managed_fields`/`apply_imported_identity`/`open_live_view`/`aushang_html`, `aushang.rs` `link_mit_guid`, `src/types.ts`, `src/io/liveLink.mjs`, GUID-Feld im `SetupWizard`, `App.tsx` Wizard-Gate, `pages/CourtMonitorPanel.tsx`; badhub-Seite: Kind-Turniere in `liveticker_tournaments`) | `docs/features/liveticker-mehrere-turniere-je-verband.md` (Spec) · `docs/aushang.md` · `docs/court-monitor.md` · `docs/spieler-check-in.md` · ADR 0054 |
...
| **Update im Turnierbetrieb** (`update.rs` `ResumeMarker`/`take_resume`/`SILENT_INSTALLER_ARGS`/`UpdateManager`, `commands.rs` `update_check`/`update_info`/`update_install_now`/`update_set_install_on_exit`/`take_update_resume`/`update_beim_beenden`, `lib.rs` `beenden`, `tablet/state.rs` `on_court_seeded` in `reconcile_on_court`, `components/UpdateBanner.tsx`, `wiederAnlaufen` in `App.tsx`) | `docs/features/update-im-turnierbetrieb.md` (Spec) · `docs/release.md` (Ablauf) · ADR 0057 |
| **Akkustand am Tablet** (`src/io/akku.mjs` + `scripts/test-akku.mjs`, Inline-Kopien + `#akku` in `assets/tablet.html` (`setupBattery`/`renderAkku`, speist auch den `battery`-Frame), `assets/lobby.html`, `assets/anzeige.html`) | `docs/tablet.md` („Akkustand am Tablet") · `docs/tablet-android-app.md` (Brücke) |
| **Tablet-Kiosk-App (Android)** (`android/` — `suche/` Subnetz/Scanner/ServerSuche, `kern/` Huelle/AdressFilter/LogPuffer/PinRegel/GeraeteId/SperrRegel/Entschlackung (+ Drift-Test `scripts/test-entschlackung-liste.mjs` gegen `setup-tablet.*`), `netz/` Sonde/mDNS/NetzBeobachter/LogUpload, `kiosk/` Kiosk/FullyBruecke/Receiver/BestaetigungsDienst (+ `kern/FixierDialog`), `KioskActivity`, `setup-tablet.*`, Jobs `android` in `ci.yml`/`release.yml`) | `docs/tablet-android-app.md` (Bedienung) · `docs/features/tablet-android-kiosk-app.md` (Spec) · `docs/tablet-kiosk.md` · ADR 0058 |

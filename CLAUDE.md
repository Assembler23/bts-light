...
| **Spielzeiten & Prognose** (`tablet/match_times.rs` (Store `match-times.json`, E4-Reset), `sync.rs` `reconcile_match_times`, E2-Stempel in `server.rs` `handle_score`, E3-Stempel + Duration-Quelle in `process_result`/`commands.rs` `enter_result`/`disqualify_match`/`tl.rs` `execute_result_action`, Walkover bewusst 0) | `docs/features/spielzeiten-prognose.md` (Spec) · `docs/btp_protocol.md` (Duration) · ADR 0027 |

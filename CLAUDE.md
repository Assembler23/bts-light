...
| **Turnierleitungs-Oberfläche (TL-Web)** (`relay-proto` `Tl*`-Typen/Frames, `config.rs` `TlWebConfig`/`TlDevice`, `commands.rs` `identity_bundle`/`keep_host_managed_fields`, `tablet/tl.rs`, `tablet/assign.rs`, `assets/tl.html`, `src/io/tl*.mjs`, TL-Routen in `server.rs` + `relay/`) | `docs/features/turnierleitung-web.md` (Spec) · `docs/cloud-relay.md` (Wire-Ebene, Token) |

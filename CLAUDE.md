...
| **Zähltafel + Anzeige-Hülle** (`assets/tafel.html` (Anordnung `nebeneinander`/`uebereinander` + `auto` nach Geräte-Ausrichtung, `?anordnung=`), `src/io/tafelSeiten.mjs` (`tafelSeiten`, `anordnungAusQuery`, `effektiveAnordnung`) + `scripts/test-tafel-seiten.mjs`, `tafel_page`/`render_tafel_html` + `CourtTafel`-Arm in `tablet/server.rs` **und** `relay/`, `relay-proto` `MonitorTarget::CourtTafel`, Option `tafel:<id>` in `pages/CourtMonitorPanel.tsx`; PR 2: `assets/anzeige.html`, `src/io/anzeigeZiel.mjs` + `scripts/test-anzeige-ziel.mjs`, `anzeige_page`/`render_anzeige_html`/`bedien_pin` in `tablet/server.rs`, `/{ns}/anzeige` + `occupied` in `courts_list` in `relay/`, Einstiege in `assets/tablet.html`) | `docs/features/zaehltafel-anzeige-huelle.md` (Spec) · `docs/court-monitor.md` (Layout, Zuweisung) · `docs/tablet.md` (Hülle, PR 2) · `docs/cloud-relay.md` (Route, Deploy-Reihenfolge) · ADR 0055 |

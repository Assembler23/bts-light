...
| **Mehrere Liveticker je Verband** (`config.rs` `Config.tournament_uuid` + Migration aus `CheckinConfig`, `badhub/payload.rs` `tournament_uuid` in `Tset`/`Tupdate`/`Sched`/`CheckinBranding`, `sync.rs`/`commands.rs` Startprüfung, `aushang.rs` `&g=`-Link, GUID-Feld im `SetupWizard`; badhub-Seite: Kind-Turniere in `liveticker_tournaments`) | `docs/features/liveticker-mehrere-turniere-je-verband.md` (Spec) · `docs/aushang.md` · `docs/spieler-check-in.md` · ADR 0054 |

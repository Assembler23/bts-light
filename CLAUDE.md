...
| **Schmaler Abruf je Feld** (`tablet/server.rs` `DeviceHeartbeat.court` + `feld_ausschnitt` im `health`-Handler, `relay/` `OverviewQuery` + Filter in `overview_health`) | `docs/features/monitor-livestand-push.md` (Spec, Etappe S7) · `docs/court-monitor.md` |
